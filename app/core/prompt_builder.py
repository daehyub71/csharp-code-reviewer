...
class PromptBuilder:
    ...
    # 시스템 프롬프트 (역할 정의)
    SYSTEM_PROMPT = """당신은 C# 코드 리뷰 전문가입니다. 주어진 코드 전체를 개선하여 완전히 재작성합니다.
다음 원칙을 따르세요:
1. 원본 코드의 모든 클래스, 메서드, 필드를 포함하여 전체를 재작성
2. 발견된 모든 문제점을 수정 (Null 참조, Exception 처리, 리소스 관리, 성능, 보안, 네이밍, 하드코딩 등)
3. C# 모범 사례 준수 및 실용적인 개선
4. 명확하고 간결한 코드 작성
5. 성능과 가독성 균형 유지
6. 반드시 C# 문법으로만 출력 (Python, Java 등 다른 언어 사용 금지)"""
...
조건문, 반복문, 메서드 호출을 명확히 표시하세요."""
    ...
    def _get_output_instruction(self, output_format: OutputFormat) -> str:
        ...
        """
        instructions = {
            OutputFormat.IMPROVED_CODE: """원본 코드 전체를 개선하여 완전히 재작성하세요.

규칙:
1. 원본 코드의 모든 클래스, 메서드, 필드를 포함하여 전체를 재작성
2. 발견된 모든 문제점을 수정 (Null 참조, Exception 처리, 리소스 관리, 성능, 보안, 네이밍, 하드코딩 등)
3. using 문, namespace, 주석 등 모든 요소 포함
4. 코드만 출력하고 설명이나 마크다운은 제외
5. 원본 코드와 동일한 기능을 유지하되 개선된 버전으로 작성
