...
⚠️ 중요: 입력된 코드의 구조를 유지하세요!
- 새로운 예제 코드를 작성하지 마세요
- 클래스/메서드 이름을 그대로 유지하세요
- 입력된 코드가 10줄이면, 개선된 코드도 비슷한 길이여야 합니다
- 예: "class Leaky"가 입력되면 → "class Leaky"를 개선하세요 (다른 클래스로 바꾸지 마세요)

다음 원칙을 따르세요:
1. 입력된 코드의 모든 클래스, 메서드, 필드를 그대로 포함 (이름 유지)
2. 발견된 문제점만 수정 (Null 참조, Exception 처리, 리소스 관리, 성능, 보안, 네이밍, 하드코딩 등)
3. C# 모범 사례 적용 (using, try-catch, null 체크 등)
4. 원본 코드와 동일한 기능 유지
...
⚠️ 절대 규칙:
- 입력된 코드의 클래스/메서드 이름을 그대로 유지하세요
- 새로운 예제 코드를 작성하지 마세요
- 입력된 코드 구조를 바꾸지 마세요

개선 규칙:
1. 입력된 코드의 모든 클래스, 메서드, 필드를 그대로 포함 (이름 변경 금지)
2. 발견된 문제점만 수정 (Null 체크, using 추가, try-catch 추가 등)
3. using 문, namespace, 주석 등 필요한 요소만 추가
4. 코드만 출력 (설명이나 마크다운 없이 순수 C# 코드만)
5. 입력된 코드와 비슷한 길이로 작성 (10줄 입력 → 15-20줄 출력)
6. 반드시 유효한 C# 문법으로만 출력 (Python, Java 등 다른 언어 사용 금지)

예시:
입력: "class Leaky { void Process() { } }"
출력: "class Leaky { void Process() { using (...) { } } }" (이름 유지!)""",
