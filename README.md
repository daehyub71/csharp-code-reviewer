...
- ✅ **6가지 리뷰 항목**: Null 참조, Exception, 리소스 해제, 성능, 보안, 네이밍 컨벤션
- ✅ **사용자 친화적 GUI**: PySide6 네이티브 인터페이스 (Before/After Split Editor)
- ✅ **Markdown 리포트**: GitHub 스타일 리포트 + 플로우 다이어그램 자동 생성

---

## 스크린샷

### 메인 화면 (Before/After Split Editor)
```
┌─────────────────────────────────────────────────────────┐
│  C# Code Reviewer - Phi-3-mini                    [-][□][X] │
├─────────────────────────────────────────────────────────┤
│ 파일(F)  편집(E)  도구(T)  도움말(H)                        │
├─────────────────────────────────────────────────────────┤
│ 입력 방식:  ● 텍스트 붙여넣기  ○ 파일 업로드  ○ 폴더 선택   │
├───────────────────────┬─────────────────────────────────┤
│  Before (원본 코드)    │   결과 미리보기                  │
├───────────────────────┤   (GitHub 스타일 Markdown)       │
│ using System;         │                                 │
│                       │   # 코드 리뷰 결과               │
│ public class Test     │                                 │
│ {                     │   ## 발견된 이슈                 │
│     var x = null;     │   - [심각] Null 참조 가능성      │
│     Console.Write(x); │                                 │
│ }                     │   ## 개선 코드                   │
│                       │   ```csharp                     │
├───────────────────────┤   var x = "value";              │
│  After (개선 코드)     │   ```                           │
├───────────────────────┤                                 │
│ using System;         │   ## 플로우 다이어그램           │
│                       │   [PNG 다이어그램 표시]          │
│ public class Test     │                                 │
│ {                     │                                 │
│     var x = "value";  │   [Markdown 저장]                │
│     Console.Write(x); │                                 │
│ }                     │                                 │
└───────────────────────┴─────────────────────────────────┘
│ 상태: 준비 완료 | Ollama: 실행 중 | 메모리: 2.3GB          │
└─────────────────────────────────────────────────────────┘
```
...
### Phase 1: MVP (현재 - Week 2)
- [x] Before/After Split Editor
- [x] 6가지 코드 리뷰 항목
- [x] Markdown 리포트 생성
- [x] Phi-3-mini 통합

### Phase 2: 파일 업로드 (Week 3)
- [ ] 파일 선택 UI
- [ ] 다중 파일 분석
- [ ] 드래그 앤 드롭

### Phase 3: 폴더 선택 (Week 4)
- [ ] 트리 구조 UI
- [ ] 대용량 프로젝트 처리
- [ ] 통합 리포트
